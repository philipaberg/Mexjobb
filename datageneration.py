    ...
    try:
        ...
        for eta in ETA_LIST:
            ...
            for gamma in GAMMA_LIST:
                # True J
                R_hat_trueJ = recover_reportings(P_full, B_total, R_total, P_total, occ_start, n_occ, max(J_P, J), J, gamma)
                _, CL_trueJ = chain_ladder(build_triangle(R_hat_trueJ, n_occ, max(J_P, J) + 1, t_eval), n_occ, max(J_P, J) + 1)
